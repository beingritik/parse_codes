...
# output_csv_file_path = env['OUTPUT_CSV_NEW_500']
master_folder = r"D:\python3\newformat"

output_csv_file_path = f"{output_path}\output__{formatted_datetime}.csv"
empty_entries_csv_path = f"{open_path}\open__{formatted_datetime}.csv"
global_csv_path = f"D:\python3\global\output_main.csv"
...
with open(global_csv_path, 'a', newline='', encoding='utf-8') as global_csv_file:
    csv_writer_global = csv.writer(global_csv_file)

    # Write header only if the file is empty
    if os.path.getsize(global_csv_path) == 0:
        header_global = list(next(iter(final_dict.values())).keys())
        header_global.insert(0, 'Bid pdf name:')  # Add 'Key' column to the beginning
        csv_writer_global.writerow(header_global)

    # Write final_dict rows
    for key, inner_dict in final_dict.items():
        row_global = [key] + list(inner_dict.values())
        csv_writer_global.writerow(row_global)
print(f"Data has been appended to {global_csv_path}")
